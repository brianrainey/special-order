...
function setContentUrl() {
  var selectedText = selection.text.trim().toLowerCase();
 ...
 */
function isSynonym(selectedText, cardName) {
  for each (synonym in synonymList[cardName]) {
    if (synonym.toLowerCase() === selectedText) {
      return true;
    }
  }
  return false;
